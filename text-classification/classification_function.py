...
def evaluate_model(data_df, prompt_template, model_name, output_file):
    ...
    for token in api_tokens:
        ...
        for index, row in batch.iterrows():
            ...
            while retry:
                try:
                    predicted_label = client.text_generation(prompt_template.format(sentence=sentence), max_new_tokens=3,
                                                             temperature=0.1)
        ...
        print(f"Token {token} - Processed {processed_index + len(batch_predictions)} out of {len(data_df)} sentences.")
