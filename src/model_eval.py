...
from conventional_models.preprocessing_conventional import preprocess_text_conventional
from lstm.dataset_setup import setup_loaders
from lstm.preprocessing_lstm import preprocess_text_lstm
from lstm.model_definition import GDPRTextClassifier
...
def evaluate_baseline_models(train_set, test_set, scenario, dataset_name, run_id):
    """
        Evaluate baseline models on the provided datasets.
    ...
    """
    X_train, y_train = preprocess_data(train_set, preprocess_text_conventional)
    X_test, y_test = preprocess_data(test_set, preprocess_text_conventional)

    for model_name, model in models.items():
        y_pred = train_and_predict_conventional(model, X_train, y_train, X_test)
...
def evaluate_lstm_model(train_set, val_set, scenario, dataset_name, run_id):
    ...
    model = GDPRTextClassifier(len(vocab), EMBEDDING_DIM, HIDDEN_DIM, DROPOUT)
