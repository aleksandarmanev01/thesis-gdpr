...
from model_eval import evaluate_lstm_model, evaluate_baseline_models
from data_config import *

RUN_ID = "Run_5"
log("Reading and preprocessing original GDPR dataset...")

# Load and preprocess data
data_df = pd.read_csv(DATASET_PATH, sep='\t').dropna()

train_data, test_data = train_test_split(data_df, test_size=0.3, random_state=42, stratify=data_df['DPO'])

for scenario, files in scenarios.items():
    # If the scenario is one of the specified, first evaluate on the original dataset
    if scenario in ['Scenario_OAC_O', 'Scenario_OGC_O']:
        dataset_name = "original"
        log(f"Processing {dataset_name} for {scenario}...")

        evaluate_baseline_models(train_data.copy(), test_data.copy(), scenario, dataset_name, RUN_ID)
    ...
    for file_path in files:
        ...
        evaluate_baseline_models(new_train_data.copy(), new_test_data.copy(), scenario, dataset_name, RUN_ID)
